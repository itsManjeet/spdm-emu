...
if [ -n "$CLEANUP" ] ; then
    ...
    tpm2_evictcontrol -C o -c 0x81000010 || true
...
tpm2_evictcontrol -C o -c root_ca.ctx 0x81000010

tpm2_flushcontext --transient-object 2>/dev/null || true
tpm2_flushcontext --loaded-session 2>/dev/null || true

echo "Creating root keys..."
tpm2_create -C 0x81000010 -G ecc -u root_ca.pub -r root_ca.priv

echo "Loading root key..."
tpm2_load -C 0x81000010 -u root_ca.pub -r root_ca.priv -c root_ca_key.ctx
...
tpm2_evictcontrol -C o -c requester.ctx 0x81000011

tpm2_flushcontext --transient-object 2>/dev/null || true
tpm2_flushcontext --loaded-session 2>/dev/null || true

echo "Creating requester keys..."
tpm2_create -C 0x81000011 -G ecc -u requester.pub -r requester.priv

echo "Loading requester key..."
tpm2_load -C 0x81000011 -u requester.pub -r requester.priv -c requester_key.ctx
...
tpm2_evictcontrol -C o -c responder.ctx 0x81000012

tpm2_flushcontext --transient-object 2>/dev/null || true
tpm2_flushcontext --loaded-session 2>/dev/null || true

echo "Creating responder keys..."
tpm2_create -C 0x81000012 -G ecc -u responder.pub -r responder.priv

echo "Loading responder key..."
tpm2_load -C 0x81000012 -u responder.pub -r responder.priv -c responder_key.ctx
